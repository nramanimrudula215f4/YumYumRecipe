...
class YumYumActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val viewModel = ViewModelProvider(this).get(YumYumViewModel::class.java)
        setContent {
            MyApplicationTheme {
                ...
                Surface(
                    ...
                    val YumYumState= viewModel.yummy.collectAsState()
                    val recipes = YumYumState.value
//                    val recipes = YumYumList()
                    YumYumScreen(recipes, onItemClicked = {id ->
                        val procedureIntent = Intent(this, ProcedureRepository()::class.java)
                        procedureIntent.putExtra("id", id)
...
@Composable
fun YumYumItemUi(item : Recipe, onItemClicked: (String)-> Unit){
        Card (
            modifier = Modifier
                .clickable {
...
@Composable
fun YumYumScreen(recipes: List<Recipe> = emptyList(), recipe : String = "Pr", onItemClicked: (String) -> Unit) {
    Column(modifier = Modifier.background(Color.White)) {
        Text(text = "All recipes found")
        Text(text = recipe)
        LazyVerticalGrid(columns = GridCells.Fixed(2) ) {
            items(recipes) { item ->
                YumYumItemUi(item, onItemClicked)
            }
        }
    }
}
